...
% Loop over normal and relaxed statistical significances:
for a = 1 : size(alphaModes, 2)
    ...
    for t = 1 : size(pixelTypes, 2)
        ...
        for p = 1 : size(pixelPositions, 2)
            ...
            % Determine mode of alpha being used:
            alphaMode = alphaModes{a};
            ...
            for phi = phis
                ...
                for alpha = alphas
                    ...
                    % Write to .csv file:
                    csvwrite(strcat(folder, alphaMode, '\alpha', num2str(alpha), '\phi', num2str(phi), '\', pixelType, '_', pixelPos, '.csv'), [sigma_range' (err(:, 1) / nr_noise) (err(:, 2) / nr_noise) (err(:, 3) / nr_noise)])
