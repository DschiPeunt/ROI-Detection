...
\begin{align*}
	...
	&= \left( \underset{m, n}{\mathrm{AND}} \left( \mathrm{OR} (A_{(i + m, j + n)}, \underbrace{(1 - B)_{(m, n)}}_{= 0} ) \right) \right) \oplus B \\
	&= \left( \underset{m, n}{\mathrm{AND}} ( A_{(i + m, j + n)} ) \right) \oplus B \\
	&= \underset{\tilde{m}, \tilde{n}}{\mathrm{OR}} \left( \mathrm{AND} \left( \underset{m, n}{\mathrm{AND}} ( A_{(i + m - \tilde{m}, j + n - \tilde{n})} ), \underbrace{B_{(m, n)}}_{= 1} \right) \right) \\
...
\begin{align*}
	...
	&= \left( \underset{m, n}{\mathrm{OR}} \left( \mathrm{AND} (A_{(i - m, j - n)}, \underbrace{B_{(m, n)}}_{= 1} ) \right) \right) \ominus B \\
	&= \left( \underset{m, n}{\mathrm{OR}} ( A_{(i - m, j - n)} ) \right) \ominus B \\
	&= \underset{\tilde{m}, \tilde{n}}{\mathrm{AND}} \left( \mathrm{OR} \left( \underset{m, n}{\mathrm{OR}} ( A_{(i - m + \tilde{m}, j - n + \tilde{n})} ), \underbrace{(1 - B)_{(m, n)}}_{= 0} \right) \right) \\
